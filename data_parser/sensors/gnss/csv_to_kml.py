...
def normalize_output_name(output_name: str, suffix: str = ".kml") -> str:
    ...
    if not name:
        raise ValueError("파일 이름을 입력해야 합니다.")
...
def build_output_kml_path(output_path: str | Path, output_name: str) -> Path:
    output_dir = Path(output_path)
    output_dir.mkdir(parents=True, exist_ok=True)

    name = normalize_output_name(output_name, ".kml")
    return output_dir / f"{name}.kml"
...
def fix_csv_to_kml(
    ...
    if not input_csv.exists():
        raise FileNotFoundError(f"CSV 파일이 존재하지 않습니다: {input_csv}")
...
def csv_to_kml(
    ...
    output_kml = build_output_kml_path(
